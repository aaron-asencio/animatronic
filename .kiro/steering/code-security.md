...
- `src/animatronic.py` no longer uses `subprocess` for audio — audio runs via `AudioPlayer` (file) or `AudioStreamer` (mic) in-process. There are no shell commands to inject into for the audio path.
- Never interpolate user-supplied values directly into shell command strings. If a file path or action name originates from outside the process (CLI args, HTTP request), validate it against an explicit allowlist before use.
...
- `src/animatronic.py` and `src/controller.py` accept `--action` from the command line. The value is dispatched through an explicit `dict`/`if-elif` allowlist — this pattern must be preserved for any new entry points.
...
- ALSA config files in `src/config/alsa/` contain hardware settings; treat them as infrastructure config and review changes carefully before deploying.
